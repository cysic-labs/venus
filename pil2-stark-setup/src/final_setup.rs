...
pub fn gen_final_setup(
    ...
    // Write binary files using native Rust writer
    {
        ...
        // Cross-artifact consistency check (plan §6.3). The shared helper
        // validates max referenced column ids in expressionsinfo against
        // starkinfo, `.exec` size against `starkinfo.mapSectionsN.cm1`, and
        // that `.bin` / `.verifier.bin` were written non-empty.
        let exec_header = (plonk_result.exec[0], plonk_result.exec[1]);
        crate::validation::validate_final_artifacts(
            files_dir.as_path(),
            "vadcop_final",
            Some(exec_header),
        )?;
